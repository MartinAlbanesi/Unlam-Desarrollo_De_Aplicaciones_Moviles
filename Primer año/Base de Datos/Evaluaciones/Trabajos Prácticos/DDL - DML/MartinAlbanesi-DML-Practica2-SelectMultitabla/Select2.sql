...
Select cod_pel, titulo, abreviatura, cod_genero, descripcion
From Pelicula
JOIN GeneroPelicula
ON Pelicula.cod_genero = GeneroPelicula.id_genero;

SELECT nombre, fecha_nac, descripcion
FROM Cliente
INNER JOIN Localidad
ON cod_localidad = id_localidad
WHERE descripcion LIKE 'San Justo'
OR descripcion LIKE 'Ramos Mejia'; 

SELECT alq.cod_pel, pel.titulo, gen.descripcion
FROM Alquiler AS alq
INNER JOIN Pelicula AS pel
ON alq.cod_pel = pel.cod_pel 
JOIN GeneroPelicula AS gen
ON gen.id_genero = pel.cod_genero
ORDER BY alq.fecha , gen.descripcion; 

SELECT alq.cod_pel, pel.titulo
FROM Alquiler AS alq
RIGHT JOIN Pelicula AS pel 
ON alq.cod_pel = pel.cod_pel 
JOIN GeneroPelicula AS gen
ON gen.id_genero = pel.cod_genero
...
ON alq.nro_doc = cli.nro_doc
WHERE DATEDIFF(alq.fecha_devolucion, alq.fecha) > 1;
...
SELECT alq.tipo_doc, alq.nro_doc, gen.descripcion
FROM alquiler AS alq
INNER JOIN Pelicula AS pel
ON alq.cod_pel = pel.cod_pel
JOIN GeneroPelicula AS gen
ON gen.id_genero = pel.cod_genero
WHERE gen.descripcion LIKE 'Comedia' ;
